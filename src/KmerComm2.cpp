...
KmerCountMap GetKmerCountMapKeys(const Vector <String>& myreads, SharedPtr<CommGrid> commgrid)
{
    ...
    *logstream << "my computed 'column' k-mer cardinality estimate is " << mycardinality;
    ...
    const size_t nreads = myreads.size();
    size_t maxsending = 0, kmersthisbatch = 0;
    size_t memthreshold = (MAX_ALLTOALL_MEM / nprocs) << 1;
    size_t offset = 0;
    do
    {
        for (; offset < nreads; ++offset)
        {
            const String& sequence = myreads[offset];
            if (sequence.size() < KMER_SIZE)
                continue;
            Vector<TKmer> repmers = TKmer::GetRepKmers(sequence);
            kmersthisbatch += repmers.size();
            for (auto meritr = repmers.begin(); meritr != repmers.end(); ++meritr)
            {
                auto& kmerbucket = kmerbuckets[GetKmerOwner(*meritr, nprocs)];
                kmerbucket.push_back(*meritr);
                maxsending = std::max(kmerbucket.size(), maxsending);
            }
            if (maxsending * TKmer::N_BYTES >= memthreshold || (kmersthisbatch + sequence.size()) * TKmer::N_BYTES >= MAX_ALLTOALL_MEM)
            {
                ++offset;
                break;
            }
        ...
        *logstream << "}";
        ...
        for (int i = 0; i < nprocs; ++i)
        {
            ...
            {
                memcpy(addrs2fill, kmerbuckets[i][j].GetBytes(), TKmer::N_BYTES);
